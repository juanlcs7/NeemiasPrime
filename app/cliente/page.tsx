...
export default async function ClientePage() {
  ...
  const [{data:profile},{data:services},{data:professionals},{data:appointments},{data:membership},{data:plans}]=await Promise.all([
    ...
    supabase.from("memberships").select("id,starts_on,ends_on,active,plans(id,name,price_cents,benefit_type,allowed_weekdays)").eq("client_id",userId).eq("active",true).maybeSingle(),
  ...
  return <ClientDashboard profile={profile} accountEmail={user.email||""} services={services||[]} professionals={professionals||[]} appointments={normalizedAppointments} membership={normalizedMembership} plans={plans||[]}/>;
}
