...
export default async function AdminPage(){const supabase=await createClient();const {data:{user}}=await supabase.auth.getUser();const userId=user?.id;if(!userId)redirect("/entrar?retorno=/admin");const {data:profile}=await supabase.from("profiles").select("role,full_name").eq("id",userId).single();if(profile?.role!=="admin")redirect("/cliente");
  ...
  return <AdminDashboard adminName={profile.full_name} appointments={appointments||[]} services={services||[]} professionals={professionals||[]} plans={plans||[]} clients={clients||[]} memberships={memberships||[]}/>;
}
