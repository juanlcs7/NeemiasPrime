...
export default async function AdminPage(){const supabase=await createClient();const {data:{user}}=await supabase.auth.getUser();const userId=user?.id;if(!userId)redirect("/entrar?retorno=/admin");const [{data:profile},{data:{session}}]=await Promise.all([supabase.from("profiles").select("role,full_name").eq("id",userId).single(),supabase.auth.getSession()]);if(profile?.role!=="admin")redirect("/cliente");if(!session?.access_token)redirect("/entrar?retorno=/admin");
  ...
  return <AdminDashboard adminName={profile.full_name} adminAccessToken={session.access_token} appointments={appointments||[]} services={services||[]} professionals={professionals||[]} plans={plans||[]} clients={clients||[]} memberships={memberships||[]}/>;
}
