...
export default function Home() {
  return <main className="landing new-landing">
    ...
    <header className="neo-nav">
      ...
      <div><Link className="nav-login" href="/entrar">Entrar</Link><Link className="lime-button" href="/entrar?retorno=/cliente">AGENDAR <span>↗</span></Link></div>
    </header>
    ...
    <section id="servicos" className="bento-services">
      ...
      <div className="service-bento">{serviceCards.map((s,index)=><article className={index===1?"featured-service":""} key={s.name}><div><span>{s.n}</span><b>{s.mark}</b></div><h3>{s.name}</h3><p>{s.note}</p><footer><strong>{s.price}</strong><Link href="/entrar?retorno=/cliente">AGENDAR ↗</Link></footer></article>)}</div>
      <Link className="service-all" href="/entrar?retorno=/cliente"><span>VER CATÁLOGO COMPLETO</span><b>Relaxamento · Reflexo · Pigmentação · Hidratação · Coloração · Limpeza de pele · e mais</b><i>↗</i></Link>
    </section>
